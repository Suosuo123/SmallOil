package com.hh.oil.widget.familiarRecyclerview;
...
/**
 * FamiliarRecyclerView Default ItemDecoration Created by iWgang on 15/11/08.
...
public class FamiliarDefaultItemDecoration extends RecyclerView.ItemDecoration {
	...
	private int mOrientation = OrientationHelper.VERTICAL; // OrientationHelper.VERTICAL
															// or
	...
	public FamiliarDefaultItemDecoration(RecyclerView recyclerView, Drawable dividerVertical,
			Drawable dividerHorizontal, int dividerDrawableSizeVertical, int dividerDrawableSizeHorizontal) {
	...
	private void drawDividerDrawable(Canvas c, RecyclerView parent) {
		...
		for (int i = 0; i < parent.getChildCount(); i++) {
			...
			if (isHeadViewPos(headersCount, position)
					|| isFooterViewPos(headersCount, footerCount, itemViewCount, position) || isEmptyView) {
				...
				if (mOrientation == OrientationHelper.HORIZONTAL) {
					...
					mVerticalDividerDrawable
							.setBounds(left + traX, parentTop + traY, right + traX, parentBottom + traY);
			...
			case FamiliarRecyclerView.LAYOUT_MANAGER_TYPE_STAGGERED_GRID:
				...
				if (mLayoutManagerType == FamiliarRecyclerView.LAYOUT_MANAGER_TYPE_GRID
						&& position == (itemViewCount + headersCount - 1)) {
				...
				if (mOrientation == OrientationHelper.HORIZONTAL) {
					...
					} else if (mLayoutManagerType == FamiliarRecyclerView.LAYOUT_MANAGER_TYPE_GRID
							&& isGridItemLayoutLastRow) {
					...
					mVerticalDividerDrawable.setBounds(verticalLeft + traX, verticalTop + traY, verticalRight + traX,
							verticalBottom + traY);
					mVerticalDividerDrawable.draw(c);
				} else {
					// draw vertical divider
					if (!isGridItemLayoutLastColumn
							&& ((mLayoutManagerType == FamiliarRecyclerView.LAYOUT_MANAGER_TYPE_GRID && !isGridLayoutLastNum) || mLayoutManagerType == FamiliarRecyclerView.LAYOUT_MANAGER_TYPE_STAGGERED_GRID)) {
					...
					int horizontalBottom = horizontalTop + mHorizontalDividerDrawableHeight;
					mHorizontalDividerDrawable.setBounds(horizontalLeft + traX, horizontalTop + traY, horizontalRight
							+ traX, horizontalBottom + traY);
					mHorizontalDividerDrawable.draw(c);
	...
	private void processGridOffsets(Rect outRect, int position, int headersCount, View view) {
		...
		final int tempDividerDrawableSize = mOrientation == OrientationHelper.HORIZONTAL ? mHorizontalDividerDrawableHeight
				: mVerticalDividerDrawableHeight;
		if (mItemViewBothSidesMargin > 0) {
			...
			mDividerRightBaseOffset = (float) tempDividerDrawableSize / mGridSpanCount
					* (mGridSpanCount - (curGridNum + 1)) + mItemViewBothSidesMargin * 2 / mGridSpanCount
					* (curGridNum + 1) - mItemViewBothSidesMargin;
		} else {
			mDividerLeftBaseOffset = (float) tempDividerDrawableSize / mGridSpanCount * curGridNum;
			mDividerRightBaseOffset = (float) tempDividerDrawableSize / mGridSpanCount
					* (mGridSpanCount - (curGridNum + 1));
		}
	...
	private boolean isInterceptFilter(int position, int headersCount, int footerCount, int itemViewCount) {
		if (isHeaderDividersEnabled && headersCount > 0 && position == 0)
			return true;

		if (!isHeaderDividersEnabled && position < headersCount)
			return true;
