package com.hh.oil.widget.guidPages;
...
public class MaterialTutorialFragment extends Fragment {
	...
	/**
	 * public void onTranform(int pageNumber, float transformation) { Log.d(TAG,
	 * "onTransform:" + transformation); if (!isAdded()) { return; } if
	 * (transformation == 0){ imageViewBack.setTranslationX(0); return; } int
	 * pageWidth = getView().getWidth(); Log.d(TAG,
	 * "onTransform Added page Width:" + pageWidth);
	 * 
	 * imageViewBack.setTranslationX(-transformation * (pageWidth / 2)); //Half
	 * the normal speed
	 * 
	 * }
	 */
