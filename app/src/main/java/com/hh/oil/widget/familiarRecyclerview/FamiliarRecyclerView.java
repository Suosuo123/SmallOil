...
/**
 * FamiliarRecyclerView Created by iWgang on 15/10/31.
...
public class FamiliarRecyclerView extends RecyclerView {
	...
	private void init(Context context, AttributeSet attrs) {
		...
		mHorizontalDividerHeight = (int) ta.getDimension(R.styleable.FamiliarRecyclerView_frv_dividerHorizontalHeight,
				-1);
		mItemViewBothSidesMargin = (int) ta.getDimension(R.styleable.FamiliarRecyclerView_frv_itemViewBothSidesMargin,
				0);
		mEmptyViewResId = ta.getResourceId(R.styleable.FamiliarRecyclerView_frv_emptyView, -1);
		isKeepShowHeadOrFooter = ta.getBoolean(R.styleable.FamiliarRecyclerView_frv_isEmptyViewKeepShowHeadOrFooter,
				false);
		...
		if (ta.hasValue(R.styleable.FamiliarRecyclerView_frv_layoutManager)) {
			int layoutManagerType = ta.getInt(R.styleable.FamiliarRecyclerView_frv_layoutManager,
					DEF_LAYOUT_MANAGER_TYPE);
			int layoutManagerOrientation = ta.getInt(R.styleable.FamiliarRecyclerView_frv_layoutManagerOrientation,
			...
			case LAYOUT_MANAGER_TYPE_GRID:
				processGridDivider(divider, dividerHeight, false, layoutManagerOrientation);
				setLayoutManager(new GridLayoutManager(context, gridSpanCount, layoutManagerOrientation,
						isReverseLayout));
				break;
	...
	private void processGridDivider(Drawable divider, int dividerHeight, boolean isLinearLayoutManager,
	...
	public void setAdapter(@SuppressWarnings("rawtypes") Adapter adapter) {
		...
		mReqAdapter = adapter;
		mWrapFamiliarRecyclerViewAdapter = new FamiliarWrapRecyclerViewAdapter(this, adapter, mHeaderView, mFooterView,
				mLayoutManagerType);
	 ...
	 * @param emptyView
	 *            your EmptyView
	 * @param isKeepShowHeadOrFooter
	 *            is Keep show HeadView or FooterView
	 */
	...
	public void addFooterView(View v, boolean isScrollTo) {
		if (mFooterView.contains(v))
			return;

		mFooterView.add(v);
		if (null != mWrapFamiliarRecyclerViewAdapter) {
			int pos = (null == mReqAdapter ? 0 : mReqAdapter.getItemCount()) + getHeaderViewsCount()
					+ mFooterView.size() - 1;
			mWrapFamiliarRecyclerViewAdapter.notifyItemInserted(pos);
			if (isScrollTo) {
				scrollToPosition(pos);
	...
	public int getFirstVisiblePosition() {
		...
		case LAYOUT_MANAGER_TYPE_LINEAR:
			ret = ((LinearLayoutManager) layoutManager).findFirstCompletelyVisibleItemPosition()
					- getHeaderViewsCount();
			break;
		case LAYOUT_MANAGER_TYPE_GRID:
	...
	private AdapterDataObserver mReqAdapterDataObserver = new AdapterDataObserver() {
		...
		public void onItemRangeMoved(int fromPosition, int toPosition, int itemCount) {
			mWrapFamiliarRecyclerViewAdapter.notifyItemMoved(getHeaderViewsCount() + fromPosition,
					getHeaderViewsCount() + toPosition);
		}
	};
