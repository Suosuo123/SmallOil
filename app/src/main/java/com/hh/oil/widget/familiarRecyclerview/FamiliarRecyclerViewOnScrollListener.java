...
/**
 * Familiar OnScrollListener Created by iWgang on 15/11/13.
...
public abstract class FamiliarRecyclerViewOnScrollListener extends RecyclerView.OnScrollListener {
	...
	private int mLayoutManagerType = 1; // 1 LinearLayoutManager, 2
										// GridLayoutManager, 3
										// StaggeredGridLayoutManager
	private boolean isCanScrolledCallback = false;
	private int[] mStaggeredFirstPositions, mStaggeredLastPositions;

	private int callbackType = 0; // 0 not callback, 1 scrolled to top, 2
									// scrolled to bottom
	...
	private boolean checkToBottom(RecyclerView recyclerView) {
		switch (mLayoutManagerType) {
		case 1:
			return mLinearLayoutManager.findLastCompletelyVisibleItemPosition() == recyclerView.getAdapter()
					.getItemCount() - 1;
		case 2:
			return mGridLayoutManager.findLastCompletelyVisibleItemPosition() == recyclerView.getAdapter()
					.getItemCount() - 1;
		case 3:
